...
@section('contents')
    <div class="container-fluid pt-4 px-4">
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <h5 class="text-dark">Data Skala Usaha</h5>
    ...
    <!-- Modal -->
    <div class="modal fade" id="skalaModal" tabindex="-1">
    ...
    <script>
        $(document).ready(function($) {
            ...
            $('.btn-add').on('click', function() {
                $.ajax({
                    method: 'get',
                    url: `{{ url('admin/skala-usaha/create') }}`,
                    success: function(res) {
                        $('#skalaModal').find('.modal-dialog').html(res)
                        modal.show()
                        store()
                    }
                })
                ...
                function store() {
                    $('#formAction').on('submit', function(e) {
                        ...
                        $.ajax({
                            method: 'POST',
                            url: `{{ url('admin/skala-usaha') }}`,
                            data: formData,
                            processData: false,
                            contentType: false,
                            success: function(res) {
                                window.LaravelDataTables["skalausaha-table"].ajax
                                    .reload()
                                modal.hide()
            ...
            const modal = new bootstrap.Modal('#skalaModal')

            $('#skalausaha-table').on('click', '.action', function() {
                let data = $(this).data()
                let id = data.id
                ...
                $.ajax({
                    method: 'get',
                    url: `{{ url('admin/skala-usaha/') }}/${id}/edit`,
                    success: function(res) {
                        $('#skalaModal').find('.modal-dialog').html(res)
                        modal.show()
                        store()
