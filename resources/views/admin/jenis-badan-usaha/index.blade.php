...
@section('contents')
    <div class="container-fluid pt-4 px-4">
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <h5 class="text-dark">Data Jenis Badan Usaha</h5>
    ...
    <!-- Modal -->
    <div class="modal fade" id="JenisBadanModal" tabindex="-1">
    ...
    <script>
        $(document).ready(function($) {
            ...
            $('.btn-add').on('click', function() {
                $.ajax({
                    method: 'get',
                    url: `{{ url('admin/jenis-badan-usaha/create') }}`,
                    success: function(res) {
                        $('#JenisBadanModal').find('.modal-dialog').html(res)
                        modal.show()
                        store()
                    }
                })
                ...
                function store() {
                    $('#formAction').on('submit', function(e) {
                        ...
                        $.ajax({
                            method: 'POST',
                            url: `{{ url('admin/jenis-badan-usaha') }}`,
                            data: formData,
                            processData: false,
                            contentType: false,
                            success: function(res) {
                                window.LaravelDataTables["jenisbadanusaha-table"].ajax
                                    .reload()
                                modal.hide()
            ...
            const modal = new bootstrap.Modal('#JenisBadanModal')

            $('#jenisbadanusaha-table').on('click', '.action', function() {
                let data = $(this).data()
                let id = data.id
                ...
                if (jenis == 'delete') {
                    if (confirm("Are You sure want to delete this Post!") === true) {
                        $.ajax({
                            method: 'DELETE',
                            url: `{{ url('admin/jenis-badan-usaha/') }}/${id}`,
                            success: function(res) {
                                window.LaravelDataTables["jenisbadanusaha-table"].ajax
                                    .reload()
                ...
                $.ajax({
                    method: 'get',
                    url: `{{ url('admin/jenis-badan-usaha/') }}/${id}/edit`,
                    success: function(res) {
                        $('#JenisBadanModal').find('.modal-dialog').html(res)
                        modal.show()
                        store()
