    ...
    <div class="container-fluid pt-4 px-4">
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <h5 class="text-dark">Data Jenis Usaha</h5>
    ...
    <script>
        $(document).ready(function($) {
            ...
            $('.btn-add').on('click', function() {
                $.ajax({
                    method: 'get',
                    url: `{{ url('admin/jenis-usaha/create') }}`,
                    success: function(res) {
                        $('#jenisModal').find('.modal-dialog').html(res)
                        modal.show()
                        store()
                    }
                })
                ...
                function store() {
                    $('#formAction').on('submit', function(e) {
                        ...
                        $.ajax({
                            method: 'POST',
                            url: `{{ url('admin/jenis-usaha') }}`,
                            data: formData,
                            processData: false,
                            contentType: false,
                            success: function(res) {
                                window.LaravelDataTables["jenisusaha-table"].ajax
                                    .reload()
                                modal.hide()
            ...
            const modal = new bootstrap.Modal('#jenisModal')

            $('#jenisusaha-table').on('click', '.action', function() {
                let data = $(this).data()
                let id = data.id
                ...
                $.ajax({
                    method: 'get',
                    url: `{{ url('admin/jenis-usaha/') }}/${id}/edit`,
                    success: function(res) {
                        $('#jenisModal').find('.modal-dialog').html(res)
                        modal.show()
                        store()
